...
func (f *Field) validateAndReturnValue(row []string, originalIndex int) (string, error) {
	for _, el := range row {
...
func (t *Table) valueExists(i int, value interface{}) bool {
...
func (t *Table) InsertNewRow(row []string) error {
	...
	for i, f := range t.Fields {
		el, err := f.validateAndReturnValue(row, i)
		if err != nil {
			return err
		}
		...
		if utils.ContainsInside(f.properties, "unique") {
			if t.valueExists(i, parsedValue) {
